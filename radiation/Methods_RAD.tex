...
With a true track length calculated one can then calculate the linear energy transfer ($LET$) of a track by dividing the total energy deposited in a cluster $E$ by the track length $T$.
...
\begin{figure}[H]
	...
	\caption{Track parameter calculation.\cite{stuartalgo}}
	\label{stuart_algo}
	\end{center}
\end{figure}
